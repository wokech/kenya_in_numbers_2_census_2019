# Kenyan Generations by Universal Classification (Laikipia)
...
# Age-sex dataset for Laikipia

county_df_age_sex_laikipia <- county_df_age_sex |>
  filter(County == "Laikipia")

county_df_age_sex_laikipia$Age <- as.numeric(county_df_age_sex_laikipia$Age)
...
county_df_age_sex_laikipia_male <- county_df_age_sex_laikipia |> select(Age, Male)
county_df_age_sex_laikipia_male$type <- 'male'
county_df_age_sex_laikipia_male$ref_year <- '2019'
k_pop_male <- county_df_age_sex_laikipia_male |> 
  rename(
    age = Age,
    population = Male,
    type = type
...
county_df_age_sex_laikipia_female <- county_df_age_sex_laikipia |> select(Age, Female)
county_df_age_sex_laikipia_female$type <- 'female'
county_df_age_sex_laikipia_female$ref_year <- '2019'
k_pop_female <- county_df_age_sex_laikipia_female |> 
  rename(
    age = Age,
    population = Female,
    type = type
...
county_df_age_sex_laikipia_total <- county_df_age_sex_laikipia |> select(Age, Total)
county_df_age_sex_laikipia_total$type <- 'total'
county_df_age_sex_laikipia_total$ref_year <- '2019' # reference year = 2019

k_pop_total <- county_df_age_sex_laikipia_total |> 
  rename(
    age = Age,
    population = Total,
    type = type
...
ggsave("sub_pro_2_pop_gen/images/county/laikipia/knbs_pop_generation_2019_universal_1.png", width = 12, height = 12, dpi = 300)
...
ggsave("sub_pro_2_pop_gen/images/county/laikipia/knbs_pop_generation_2019_universal_2.png", width = 12, height = 12, dpi = 300)
...
ggsave("sub_pro_2_pop_gen/images/county/laikipia/knbs_pop_generation_2019_universal_3.png", width = 12, height = 12, dpi = 300)
