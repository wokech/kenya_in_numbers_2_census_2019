# Kenyan Generations by Universal Classification (Migori)
...
# Age-sex dataset for Migori

county_df_age_sex_migori <- county_df_age_sex |>
  filter(County == "Migori")

county_df_age_sex_migori$Age <- as.numeric(county_df_age_sex_migori$Age)
...
county_df_age_sex_migori_male <- county_df_age_sex_migori |> select(Age, Male)
county_df_age_sex_migori_male$type <- 'male'
county_df_age_sex_migori_male$ref_year <- '2019'
k_pop_male <- county_df_age_sex_migori_male |> 
  rename(
    age = Age,
    population = Male,
    type = type
...
county_df_age_sex_migori_female <- county_df_age_sex_migori |> select(Age, Female)
county_df_age_sex_migori_female$type <- 'female'
county_df_age_sex_migori_female$ref_year <- '2019'
k_pop_female <- county_df_age_sex_migori_female |> 
  rename(
    age = Age,
    population = Female,
    type = type
...
county_df_age_sex_migori_total <- county_df_age_sex_migori |> select(Age, Total)
county_df_age_sex_migori_total$type <- 'total'
county_df_age_sex_migori_total$ref_year <- '2019' # reference year = 2019

k_pop_total <- county_df_age_sex_migori_total |> 
  rename(
    age = Age,
    population = Total,
    type = type
...
ggsave("sub_pro_2_pop_gen/images/county/migori/knbs_pop_generation_2019_universal_1.png", width = 12, height = 12, dpi = 300)
...
ggsave("sub_pro_2_pop_gen/images/county/migori/knbs_pop_generation_2019_universal_2.png", width = 12, height = 12, dpi = 300)
...
ggsave("sub_pro_2_pop_gen/images/county/migori/knbs_pop_generation_2019_universal_3.png", width = 12, height = 12, dpi = 300)
