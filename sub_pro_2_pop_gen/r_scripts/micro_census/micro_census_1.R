...
pop_micro_census <- pop_micro_census |>
  ...
  mutate(hierarchy_levels = case_when(
...
pop_micro_census <- pop_micro_census |>
  ...
  select(Name, "Leading Spaces", "Hierarchy Levels", everything())
...
pop_micro_census_processed <- pop_micro_census_processed |> 
  select(name, hierarchy_levels, full_name, c(total:population_density_per_sq_km), everything()) |>
